    ...
    <div>
        <div class="uk-form-group">
            ...
            <div class="uk-position-relative w-100">
                ...
                <input id="email" type="text" name="email" class="uk-input @error('email') is-invalid @enderror"
                    placeholder="Votre adresse email" value="{{ old('email') }}" required autocomplete="email"
                    autofocus>
            </div>
            @error('email')
    ...
    <div>
        <div class="mt-4 uk-flex-middle uk-grid-small" uk-grid>
            <div class="uk-width-expand@s">
                <p>Pas encore membre ? <br><a class="font-weight-bold text-primary" href="{{ route('register') }}">Créer
                        un compte</a></p>
            </div>
...
<script>
    let pwd = document.getElementById('password');
        let d_pwd = document.getElementById('d_pwd');
        d_pwd.addEventListener('click',()=> {
            if(d_pwd.checked){
            return  pwd.type = 'text';
            } 
            return pwd.type = 'password';
        } )
