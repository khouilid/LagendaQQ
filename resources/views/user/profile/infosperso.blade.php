...
<div class="row">
    <div class="my-4 col-12 row">
        ...
        <div class="col-11">
            ...
            <h5 class="my-0 bold"><small>Le numéro de telephone :</small> {{$user->mobile_phone}}</h5>
            <h5 class="my-0 bold"><small>Le numéro civique :</small>{{$user->num_civique}}</h5>
            <div><span class="my-0"><small>Email : </small>{{$user->email}}</span></div>
            <small>Compte crée {{$user->created_at}}</small>
        </div>
    </div>
    <nav class="col-12">
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
            <a class="nav-link {{@$default_tab=='account'?'active':''}}" id="nav-account-tab" data-bs-toggle="tab"
                href="#nav-account" role="tab" aria-controls="nav-account" aria-selected="true"><i
                    class="fa fa-cogs"></i> Mon Compte</a>
            <a class="nav-link {{@$default_tab=='wallet'?'active':''}}" id="nav-wallet-tab" data-bs-toggle="tab"
                href="#nav-wallet" role="tab" aria-controls="nav-wallet" aria-selected="false"><i
                    class="fa fa-wallet"></i> Mon portefeuille </a>
            <a class="nav-link {{@$default_tab=='transactions'?'active':''}}" id="nav-transactions-tab"
                ...
                aria-selected="false"><i class="fa fa-list"></i> Mes transferts</a>
            <a class="nav-link {{@$default_tab=='infos-perso'?'active':''}}" id="nav-infos-perso-tab"
                ...
                aria-selected="false"><i class="fa fa-user"></i> Informations Personelles</a>
            <a class="nav-link {{@$default_tab=='security'?'active':''}}" id="nav-security-tab" data-bs-toggle="tab"
                href="#nav-security" role="tab" aria-controls="nav-security" aria-selected="false"><i
                    class="fa fa-user-lock"></i> Sécurité </a>
            {{-- <a class="nav-link {{@$default_tab=='functions'?'active':''}}" id="nav-security-tab"
            data-bs-toggle="tab" href="#nav-security" role="tab" aria-controls="nav-security" aria-selected="false"><i
                class="fa fa-cogs"></i> </a> --}}
            <a class="nav-link {{@$default_tab=='events'?'active':''}}" id="nav-events-tab" data-bs-toggle="tab"
                href="#nav-events" role="tab" aria-controls="nav-events" aria-selected="false"><i
                    class="fa fa-list"></i> Mes possibles</a>
        </div>
    </nav>
    <div class="col-12 tab-content" id="nav-tabContent">
        <div class="tab-pane fade {{@$default_tab=='account'?'show active':''}}" id="nav-account" role="tabpanel"
            aria-labelledby="nav-account-tab">
            @include("user.profile.includes.roleDisc")
        </div>
        <div class="tab-pane fade {{@$default_tab=='events'?'show active':''}}" id="nav-events" role="tabpanel"
            aria-labelledby="nav-events-tab">
            @include("user.profile.includes.account_fonctions")
        </div>
        <div class="tab-pane fade {{@$default_tab=='wallet'?'show active':''}}" id="nav-wallet" role="tabpanel"
            aria-labelledby="nav-wallet-tab">
            <div class="bg-white">
                @include('user.profile.includes.my_wallet')
            </div>
        </div>
        <div class="mt-4 tab-pane fade {{@$default_tab=='transactions'?'show active':''}}" id="nav-transactions"
            role="tabpanel" aria-labelledby="nav-transactions-tab">
            @include('user.profile.includes.my_transactions')
        </div>
        <div class="mt-4 tab-pane fade {{@$default_tab=='infos-perso'?'show active':''}}" id="nav-infos-perso"
            role="tabpanel" aria-labelledby="nav-infos-perso-tab">
            <div class="bg-white">
        ...
        <div class="mt-4 tab-pane fade {{@$default_tab=='security'?'show active':''}}" id="nav-security" role="tabpanel"
