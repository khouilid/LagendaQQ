    ...
    @if(session()->get('role') !== null)
            <div class="row g-0">
                <div class="col-md-4">
                    ...
                    {!! session()->get('role')->free_events ? "<span class='badge badge-success mx-auto'><i class='fa fa-dot-circle'></i> Activités gratuites</span>":'' !!} 
                    {!! session()->get('role')->free_annoncements ? "<span class='badge badge-success mx-auto'><i class='fa fa-dot-circle'></i> Annonces gratuites</span>":'' !!}
                    <ul class="list-group">
                        <li class="list-group-item">
                            <strong class="card-text">{{$crs[0]["name"]}} par événement : {{intval(- session()->get('role')->events_price)}} </strong>
                        </li>
                        <li class="list-group-item">
                            <strong class="card-text">{{$crs[0]["name"]}} par annonce : {{intval(- session()->get('role')->annoucements_price)}} </strong>
                        </li>
                    </ul>
                </div>
                <div class="col-md-8">
