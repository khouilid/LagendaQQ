            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="flush-heading{{$fonction->id}}">
                    ...
                    <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                        data-bs-target="#flush-collapse{{$fonction->id}}" aria-expanded="false"
                        aria-controls="flush-collapse{{$fonction->id}}">
                        <i class="nav-icon far fa-user"></i>
                        &nbsp; {{ucfirst($fonction->name)}} </button>
                    @endrole
                </h2>
                <div id="flush-collapse{{$fonction->id}}" class="accordion-collapse collapse"
                    aria-labelledby="flush-heading{{$fonction->id}}" data-bs-parent="#accordionFlushExample">
                    ...
                    <div class="align-items-center d-flex">
                        ...
                        <p class="btn btn-outline-danger rounded-lg">{!! $fonction->posts - $fonction->users_count !!}
                        </p>

                        @else
                        <p class="btn btn-outline-danger rounded-lg">Aucun posts desponible</p>
                    ...
                    <div class="aq-card-action" id="{{$fonction->id}}">
                        ...
                        <a href="#" class="btn btn-outline-primary btn-sm subscribe_role"
                            data-role_id="{{$fonction->id}}" data-role_description="{!! $fonction->description !!}"
                            data-role_name="{{$fonction->name}}"><i class="fa fa-plus"></i> S'inscrire à cette
                            fonction</a>
        ...
        <div class="modal-content">
            <form action="{{route('user.assign_role')}}" method="post">
                ...
                <div class="modal-header">
                    <h4 class="modal-title text-center" id="exampleModalLabel">Vous allez vous inscrire à la fonction
                        <span id="role_name"></span></h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3 text-primary"> Confirmer votre inscription à la fonction <span
                            id="confirm_role_name"></span></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal"><i
                            class="fa fa-times"></i>
                        Close</button>
                    <button type="submit" class="btn btn-outline-primary"><i class="fa fa-check"></i> Confimer</button>
                </div>
            </form>
        </div>
    </div>
