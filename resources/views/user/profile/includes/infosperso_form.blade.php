<div class="col-12 mt-4">
    <form action="{{route('user.updateInfosPerso')}}" method="post" class="row col-md-12 custom-responsive">
        ...
        <div class="offset-sm-1 col-5 form-group row">
            ...
            <input tabindex="6" class="form-control" name="prenom" id="prenom" type="text"
        ...
        <div class="offset-sm-1 col-5 form-group row">
            ...
            <select name="region_id" id="region_id" class="form-control">
                ...
                <option value="{{$key}}" {{ (old("region_id") == $key || $user->region_id == $key) ? "selected":"" }}>
                    {{$region}}</option>
                @empty
                @endforelse
            </select>
        ...
        <div class="offset-sm-1 col-5 form-group row">
            ...
            <div class="btn-group" role="group" aria-label="Basic radio toggle button group">
                <input type="radio" class="btn-check" name="gender" id="gender_m" autocomplete="off" value="1"
                    {{ intval($user->gender === 1 ) ? 'checked':"" }}>
                <label class="btn btn-outline-primary" for="gender_m"><i class="fa fa-male"></i> Masculin</label>

                <input type="radio" class="btn-check" name="gender" id="gender_f" autocomplete="off" value="0"
                    {{ intval($user->gender === 0 ) ? 'checked':"" }}>
        ...
        <div class="offset-sm-1 col-5 form-group row">
            ...
            <input class="form-control" id="num_civique" name="num_civique" type="text"
                value="{{old('num_civique', $user->num_civique)}}">
        </div>
        <div class="offset-sm-1 col-5 form-group row">
            <label for="num_tel" class="col-sm-12 col-md-12">Numéro de téléphone : </label>
            <input class="form-control phone_number" id="num_tel" name="num_tel" type="text"
                value="{{old('num_tel',$user->num_tel)}}">
        </div>
        <div class="offset-sm-1 col-5 form-group row">
            <label for="street" class="col-sm-12 col-md-12">Nom de la rue : </label>
            <input class="form-control" id="street" name="street" type="text" value="{{old('street',$user->street)}}">
        </div>
        <div class="offset-sm-1 col-5 form-group row">
            <label for="mobile_phone" class="col-sm-12 col-md-12">Cellulaire : </label>
            <input class="form-control phone_number" id="mobile_phone" name="mobile_phone" type="text"
        ...
        <div class="offset-sm-1 col-5 form-group row">
            ...
            <select name="age_group" id="age_group" class="form-control">
                ...
                <option value="{{$key}}" {{ (old("age_group") == $key || $user->age_group == $key) ? "selected":"" }}>
                    {{$age}}</option>
                @endforeach
            </select>
        </div>
