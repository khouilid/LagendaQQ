...
<div class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    ...
    <select name="category_id" id="category_id" class="form-control">
        ...
        <option value="{{$category->id}}"
            {{old('category_id',@$announcement->category_id) == $category->id?'selected':''}}> {{$category->name}}
        </option>
        @endforeach
    </select>
...
<div class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    <img id="preview" src="{{ route('show_image',@$announcement->images) }}" alt="{{@$announcement->title}}"
...
<div class="col-12 form-group">
    ...
    <div><textarea name="description" id="description"
            class="ckeditor form-control">{{old('description',@$announcement->description)}}</textarea></div>
</div>
...
<div id="price_typ" class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    ...
    <select name="price_type" id="price_type" class="form-control">
        ...
        <option value="1" {{intval(old('price_type',@$announcement->price_type)) === 1?"selected":"" }}>Entrez le prix
        </option>
...
<div id="cele" style="display: none" class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    ...
    <select name="cele_type" id="cele_type" class="form-control">
        ...
        {{-- <option value="2" {{intval(old('price_type',@$announcement->price_type)) === 2?"selected":"" }}>Échange
        </option> --}}
    </select>
    {!! $errors->first('price_type', '<div class="error-message col-12">:message</div>') !!}
</div>
...
<div id="price_field_wrapper" class="offset-sm-0 col-sm-12 col-md-6 form-group row"
    style="{{intval(old('price_type',@$announcement->price_type)) === 1?'':'display:none'}}">
    <label for="images" class="col-sm-12 col-md-12">Price : </label>
    <input type="number" name="price" id="price" min="0" step="0.01" value="{{old('price',@$announcement->price)}}"
        class="form-control" placeholder="Entrez le prix de votre annonce Ex : 7.5">
</div>
<div class="col-12">
    <hr />
</div>
...
<div class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    ...
    <select name="advertiser_type" id="advertiser_type" class="form-control">
        <option value=""> --- </option>
        <option value="1" {{intval(old('advertiser_type',@$announcement->advertiser_type)) === 1?'selected':''}}>
            Particulier </option>
        <option value="2" {{intval(old('advertiser_type',@$announcement->advertiser_type)) === 2?'selected':''}}>
            Commerce </option>
    </select>
...
<div class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    ...
    <select name="region_id" id="region_id" class="form-control">
        ...
        <option value="{{$key}}"
            {{ intval(old('region_id',@$announcement->region_id)) === intval($key) ? "selected":"" }}>{{$region}}
        </option>
        @empty
        @endforelse
    </select>
...
<div class="offset-sm-0 col-sm-12 col-md-6 form-group row">
    ...
    <select name="city_id" id="city_id" class="form-control">
        ...
        <option value="{{$key}}" {{  intval(old('city_id',@$announcement->city_id)) === intval($key) ? "selected":"" }}>
            {{$city}}</option>
        @empty
        @endforelse
    </select>
...
<div class="offset-sm-0 col-sm-12 col-md-4 form-group row">
    ...
    <input name="postal_code" id="postal_code" class="form-control"
        value="{{old('postal_code',@$announcement->postal_code)}}" />
</div>
<div class="offset-sm-0 col-sm-12 col-md-4 form-group row">
...
<div id="telephoneDev" class="offset-sm-0 col-sm-12 col-md-4 form-group row">
    ...
    <input name="telephone" id="telephone" class="form-control phone_number"
        value="{{old('telephone',@$announcement->telephone)}}" />
</div>
<div class="col-12">
    <hr>
</div>
<div class="offset-sm-8 col-sm-12 col-md-4 form-group row">
    ...
    <select name="publication_status" id="publication_status" class="form-control">
        ...
        <option value="{{$key}}"
            {{  old('publication_status',@$announcement->publication_status) == $key ? "selected":"" }}>{{$statu}}
        </option>
        @empty
        @endforelse
    </select>
