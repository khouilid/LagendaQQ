...
$("input[name='postal_code']").on('keyup',function(e){
    //get input value and delete all space and take only 6 caracters
    const postalcode = jQuery.trim( $(this).val().replace(/ /g,'') ).substring(0, 6);
    let returned_string = '';
    //Loop through string
    for ( var i = 0; i < postalcode.length; i++ )
    {
        if(i === 3){returned_string += " ";}
        returned_string += postalcode.charAt(i);
    }


    let p = document.getElementById("postal_warning");
    let test = testCanadianPostalCode(postalcode);
    if(!test){
    ...
    $(this).val(returned_string.toUpperCase());
})
